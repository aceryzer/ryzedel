...
// Added edit-message logger
import { FluxDispatcher } from "@vendetta/metro/common";
import { findByProps } from "@vendetta/metro";
const MessageStore = findByProps('getMessage','getMessages');

// Track edits
FluxDispatcher.subscribe("MESSAGE_UPDATE", (e) => {
    try {
        const msg = MessageStore.getMessage(e.message.channel_id, e.message.id);
        const oldContent = msg?.content ?? "";
        const newContent = e.message?.content ?? "";
        if (oldContent !== newContent) {
            console.log("[NoDelete EditLog]", {id: e.message.id, oldContent, newContent});
        }
    } catch(err){console.error("EditLog error", err);}
});
